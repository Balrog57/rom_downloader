...
- `.rom_downloader.sqlite` is the local SQLite catalog/download database. It is generated by `python main.py --index-catalog` and is ignored by Git.
- `dat/` subdirectories (`no-intro/`, `redump/`, `retool - french no unl/`) are section headers in the GUI DAT selector, not selectable items.
- `.env` holds API keys (1fichier, AllDebrid, RealDebrid, archive.org, `LIBTORRENT_DLL_DIR`). Copy from `.env.example`. Never commit `.env`.
